...
export function IndiaMap() {
  const indiaMap = "/assets/india-map.jpg";

  return (
    <div className="glass-panel relative overflow-hidden rounded-3xl z-10">
      <div className="flex items-center justify-between p-5">
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--lime-glow)]">
            Geospatial
          </div>
          <h2 className="mt-1 font-display text-base font-bold sm:text-lg">India Operations Map</h2>
        </div>
        <div className="hidden gap-1.5 rounded-full bg-black/25 p-1 sm:flex">
          {["Weather", "Markets", "Diseases"].map((t, i) => (
            <button
              key={t}
              className={[
                "rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider transition cursor-pointer",
      ...
      <div className="relative aspect-[6/5] w-full sm:aspect-[16/10]">
        ...
        >
          <div className="rounded-2xl border border-dashed border-white/15 bg-black/30 px-5 py-4 text-center backdrop-blur-md">
            <div className="text-mono text-[9px] uppercase tracking-widest text-white/50">
              No live map markers
            </div>
        ...
        <div className="glass-panel absolute right-4 top-4 rounded-xl px-3 py-2 text-[10px]">
          <div className="flex items-center gap-1.5 font-bold uppercase tracking-wider text-white/60">
            <MapPin className="size-3 text-emerald-400" /> states
          </div>
          <div className="mt-0.5 font-display font-bold text-white/70">villages</div>
        </div>
      </div>
    </div>
  );
