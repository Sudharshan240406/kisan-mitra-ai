...
logger = logging.getLogger("kisan_mitra_ai.sms.exotel_provider")
...
class ExotelProvider:
    ...
    def __init__(
        self,
        provider_id: str = "exotel",
        version: str = "1.0.0",
        capabilities: Optional[List[str]] = None
    ) -> None:
        self._id = provider_id
        self._version = version
        self._capabilities = capabilities or ["transactional_sms", "alerts_sms"]
    ...
    async def send_sms(self, recipient: str, body: str) -> bool:
        ...
        if sid and token:
            try:
                url = f"https://api.exotel.com/v1/Accounts/{sid}/Sms/send.json"
                async with httpx.AsyncClient(timeout=6.0) as client:
                    ...
                    if response.status_code in (200, 201):
                        logger.info(f"[ExotelProvider] Sent SMS to {recipient}")
                        self._sent_messages.append((recipient, body))
                        return True
            except Exception as e:
                logger.warning(f"[ExotelProvider] HTTP call failed: {e}")

        self.latency_ms = (time.perf_counter() - start_time) * 1000
        logger.info(f"[ExotelProvider Stub] Sent SMS to {recipient}: {body}")
        self._sent_messages.append((recipient, body))
        return True
